...
def generate_realtime_accessibility_html():
    ...
    1. New User-Defined Mathematical Model:
       - Facility Score = ln(1 + passengers / median_passengers)
       - Transit Mobility = Sum( decay_weight_i * facility_score_i )
       - Population-Adjusted Transit Mobility = Transit Mobility / ln(1 + living_population)
    2. Multi-Spatial View Modes: 250m Grid, Administrative Dong (행정동 427개), and District Gu (자치구 25개)
    3. Real-time Aggregation Metrics: Average Score, Total Sum Score, and Max Score
    4. Blue-to-Red (Cool to Warm) default color ramp
    5. Bottom 1% low-accessibility grid highlighting (Excluding 0-score and masked grids)
    6. Multiple distance decay functions: Gaussian, Exponential, Linear, and No Decay (Flat)
    7. Bulletproof Spatial Click Lookup & Collapsible Info Panel with Detailed Formula Breakdown
    """
...
<head>
    ...
    <title>서울시 250m 격자 & 행정구역 대중교통 접근성 실시간 지도 시뮬레이터</title>
...
<body>
    ...
    <header>
        <h1>🗺️ 서울시 250m 격자 & 행정구역 대중교통 접근성 지도</h1>
    ...
    <div class="main-container">
        ...
        <div class="sidebar">
            ...
            <div class="control-group">
                <select id="score-target" style="background:#0f172a; border-color:#f59e0b; font-weight:700; color:#f59e0b;">
                    <option value="pop_adjusted" selected>👥 인구를 고려한 교통이동성 [교통이동성 / ln(생활인구)]</option>
                    <option value="raw_mobility">🚌🚇 순수 교통이동성 [ ∑(거리감쇠계수 × 시설점수) ]</option>
                    <option value="bus">🚌 버스 교통이동성</option>
                    <option value="subway">🚇 지하철 교통이동성</option>
                </select>
            </div>
        ...
        <div class="info-panel" id="info-panel">
            ...
            <div id="info-content">
                <p style="color: var(--text-muted);">지도의 격자나 행정구역을 클릭하면 상세 수치와 수식 분해가 출력됩니다.</p>
            </div>
        </div>
    ...
    <script>
        ...
        // Compute City-wide Median Passengers for Buses & Subways
        ...
        function unpinGrid() {
            if (pinnedGridIndex >= 0) {
                ...
                document.getElementById('info-content').innerHTML = `<p style="color: var(--text-muted);">지도의 격자를 클릭하면 해당 격자의 상세 정보와 실시간 접근성 수치가 고정됩니다.</p>`;
            }
        }
        ...
        // USER'S NEW FACILITY SCORE FORMULA: ln(1 + passengers / median_passengers)
        function calcFacilityScore(passCount, medianPass) {
            const ratio = passCount / (medianPass || 1);
        ...
        function recalculateAccessibility() {
            ...
            // Facility scores based on ln(1 + passengers / median_passengers)
            ...
            grids.forEach((g, idx) => {
                ...
                g.busCount = busCount;
                g.subCount = subCount;
                g.rawMobility = busScore + subScore;

                // Population-Adjusted Transit Mobility: Transit Mobility / ln(1 + living_population)
                const popDenominator = (g.pop > 0) ? Math.log1p(g.pop) : 1.0;
                g.popAdjustedMobility = (g.pop > 0 && !g.masked) ? (g.rawMobility / popDenominator) : 0.0;

                let targetVal = g.popAdjustedMobility;
                if (scoreTarget === 'raw_mobility') targetVal = g.rawMobility;
        ...
        // Detailed Breakdown Box according to USER's EXACT FORMULAS
        function showGridInfo(g, isPinned) {
            ...
            container.innerHTML = `
                ...
                <div class="info-row"><label>👥 인구를 고려한 교통이동성:</label> <span style="color:#f59e0b; font-size:1.1rem; font-weight:800;">${(g.popAdjustedMobility || 0).toFixed(3)}</span></div>
                <div class="info-row"><label>🚌🚇 순수 교통이동성 (∑ W·S):</label> <span style="color:#38bdf8; font-weight:700;">${(g.rawMobility || 0).toFixed(3)}</span></div>

                <!-- FORMULA BREAKDOWN BOX -->
                <div class="breakdown-box">
                    <h4>🔍 사용자 지정 수식 분해 (Model Breakdown)</h4>

                    <div class="breakdown-item">
                        <div class="title">
                            <span>1️⃣ 버스 시설점수 & 이동성 (${busFacCount}개)</span>
                            <span class="highlight">${(g.busScore || 0).toFixed(3)}</span>
                        </div>
                        <div class="formula">
                            • 시설점수 S_i = ln(1 + 승하차인원 / ${medianBusPassengers.toLocaleString()})<br>
                            • 정류장 평균 시설점수: <span style="color:#60a5fa;">${avgBusFac.toFixed(3)}</span><br>
                            • 거리감쇠계수 합계 (∑W): ${sumBusDecayW.toFixed(2)}<br>
                            • 버스 교통이동성 = ∑(W_i × S_i) = <span class="highlight">${(g.busScore || 0).toFixed(3)}</span>
                        </div>
                    </div>

                    <div class="breakdown-item subway">
                        <div class="title">
                            <span>2️⃣ 지하철 시설점수 & 이동성 (${subFacCount}개)</span>
                            <span class="highlight">${(g.subScore || 0).toFixed(3)}</span>
                        </div>
                        <div class="formula">
                            • 시설점수 S_i = ln(1 + 승하차인원 / ${medianSubwayPassengers.toLocaleString()})<br>
                            • 지하철역 평균 시설점수: <span style="color:#c084fc;">${avgSubFac.toFixed(3)}</span><br>
                            • 거리감쇠계수 합계 (∑W): ${sumSubDecayW.toFixed(2)}<br>
                            • 지하철 교통이동성 = ∑(W_i × S_i) = <span class="highlight">${(g.subScore || 0).toFixed(3)}</span>
                        </div>
                    </div>

                    <div class="breakdown-item pop">
                        <div class="title">
                            <span>3️⃣ 인구를 고려한 최종 교통이동성</span>
                            <span class="highlight" style="color:#f59e0b;">${(g.popAdjustedMobility || 0).toFixed(3)}</span>
                        </div>
                        <div class="formula">
                            • 순수 교통이동성 = 버스(${g.busScore.toFixed(2)}) + 지하철(${g.subScore.toFixed(2)}) = <b>${g.rawMobility.toFixed(3)}</b><br>
                            • 분모 = ln(1 + 생활인구 ${g.pop.toLocaleString()}명) = <b>${lnPop.toFixed(3)}</b><br>
                            • 최종 산식 = <span style="color:#f59e0b; font-weight:700;">${g.rawMobility.toFixed(3)} / ${lnPop.toFixed(3)} = ${(g.popAdjustedMobility || 0).toFixed(3)}</span>
                        </div>
                    </div>
                </div>
            `;
        ...
        window.onload = function() {
            ...
            const inputs = ['spatial-view-mode', 'agg-metric', 'r-bus', 'r-subway', 'score-target', 'color-ramp', 'decay-mode', 'toggle-bottom1'];
            inputs.forEach(id => {
...
"""
    ...
    print(f"[AppBuilder] Successfully implemented new mathematical formula model at: {out_html}")
    return out_html
